...
/**
 * Clase BigInt Exception
 * @brief La clase nos permite enviar excepciones producidas durante la ejecución de la clase Bigint,
 *        que a su vez es heredera de la clase std::exception de la STL.
 */
class BigIntException : public std::exception {
 public: 
  ...
  const char* BigIntIntegerRepresentation() {
    return "The BigInt Class only work integer numbers!";
  }
