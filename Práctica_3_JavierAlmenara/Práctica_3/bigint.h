 ...
 * @param s : Objeto del tipo string
 */
BigInt<2>::BigInt(const std::string& s) {
  std::vector<bool> digits;
  bool is_negative = false;
  /// Comprobar si el número es negativo
  int j = 0;
 ...
 * @param out : Objeto del tipo flujo estándar de salida
 */
std::ostream& BigInt<2>::write(std::ostream& out) const {
  out << *this;
  return out;
}
 ...
 * @param in : Objeto del tipo flujo estándar de entrada
 */
std::istream& BigInt<2>::read(std::istream& in) { 
  in >> *this;
  return in;
}
 ...
 * @param b : Objeto del tipo BigInt
 */
Number* BigInt<2>::add(const Number* b) const {
 ...
 * @param b : Objeto del tipo BigInt
 */
Number* BigInt<2>::subtract(const Number* b) const {
  const auto* p = dynamic_cast<const BigInt<2>*>(this);
  if (p) {
 ...
 * @param b : Objeto del tipo BigInt
 */
Number* BigInt<2>::multiply(const Number* b) const {
  const auto* p = dynamic_cast<const BigInt<2>*>(this);
  if (p) {
    return new BigInt<2> (*this * (BigInt<2>)*b);
  }
 ...
 * @param b : Objeto del tipo BigInt
 */
Number* BigInt<2>::divide(const Number* b) const {
  const auto* p = dynamic_cast<const BigInt<2>*>(this);
  if (p) {
    return new BigInt<2> (*this / (BigInt<2>)*b);
  }
 ...
 * @param b : Objeto del tipo BigInt
 */
Number* BigInt<2>::module(const Number* b) const {
  const auto* p = dynamic_cast<const BigInt<2>*>(this);
  if (p) {
    return new BigInt<2> (*this % (BigInt<2>)*b);
  }
 ...
 * @param b : Objeto del tipo BigInt
 */
Number* BigInt<2>::power(const Number* b) const {
  const auto* p = dynamic_cast<const BigInt<2>*>(this);
  if (p) {
