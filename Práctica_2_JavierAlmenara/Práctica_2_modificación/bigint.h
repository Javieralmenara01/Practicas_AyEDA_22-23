...
class BigInt {
 public:
  ...
  BigInt(std::string&);
  BigInt(const char*);
  BigInt(const BigInt<Base, T>&); /// Constructor de copia
  /// Transformador de base
  operator BigInt<2, bool>();
  /// Asignación
  BigInt<Base ,T>& operator=(const BigInt<Base, T>&);
  /// Inserción y extracción en flujo
  friend std::ostream& operator<< <Base, T >(std::ostream&, const BigInt<Base,T>&); 
  friend std::istream& operator>> <Base, T>(std::istream&, BigInt<Base, T>&);
  /// Getters
  std::vector<char>& digits() { return digits_; }
  ...
  /// Comparación
  friend bool operator== <Base, T>(const BigInt<Base, T>&, const BigInt<Base, T> &); 
  ...
  BigInt<Base ,T>& operator++(); /// Pre-incremento
  BigInt<Base, T> operator++(int); /// Post-incremento
  BigInt<Base, T>& operator--(); /// Pre-decremento
  BigInt<Base, T> operator--(int); /// Pre-decremento 
  /// Operadores Aritméticos
  friend BigInt<Base, T> operator+ <Base, T>(const BigInt<Base, T>&, const BigInt<Base, T>&); 
  BigInt<Base ,T> operator-(const BigInt<Base, T> &) const;
  BigInt<Base ,T> operator-() const;
  BigInt<Base ,T> operator*(const BigInt<Base ,T>&) const;
  friend BigInt<Base ,T> operator/ <Base>(const BigInt<Base ,T>&, const BigInt<Base ,T>&); 
  BigInt<Base ,T> operator%(const BigInt<Base ,T>&) const;
  /// Potencia a^b
  friend BigInt<Base ,T> power <Base>(const BigInt<Base ,T>&, const BigInt<Base ,T>&);
  /// Otros
  friend BigInt<Base ,T> absolute_sum <Base>(const BigInt<Base ,T>&, const BigInt<Base ,T>&);
  friend BigInt<Base ,T> absolute_substraction <Base>(const BigInt<Base ,T>&, const BigInt<Base ,T>&);
  friend bool absolute_value <Base>(const BigInt<Base ,T>&, const BigInt<Base ,T>&);
 private:
 ...
 */
template<class T>
std::vector<T> absolute_increase(const std::vector<T>& digits_, size_t base) {
  std::vector<T> digits = digits_;
  int carry = 1;
 ...
 */
template<class T>
std::vector<T> absolute_decrease(const std::vector<T>& digits_, size_t base) {
  std::vector<T> digits = digits_;
  int carry = 1;
  for (int i = 0; i < (int)digits.size(); i++) {
    digits[i] -= carry;
...
BigInt<Base ,T> absolute_sum(const BigInt<Base ,T>& a, const BigInt<Base ,T>& b) {
  BigInt<Base ,T> c;
  c.digits_.clear();
  int carry = 0;
 ...
 */
template<size_t Base, class T>
BigInt<Base ,T> absolute_substraction(const BigInt<Base ,T>& a, const BigInt<Base ,T>& b)  {
  BigInt<Base ,T> c;
  c.digits_.clear();
  int carry = 0;
  int size_a = a.digits_.size();
 ...
 */
template<size_t Base, class T>
bool absolute_value(const BigInt<Base ,T>& a, const BigInt<Base ,T>& b) {
 ...
 */
template<size_t Base, class T>
BigInt<Base, T>::BigInt(std::string& s) {
  std::vector<T> digits;
  /// Comprobar si el número es negativo
  int j = 0;
  if (s[0] == '-') {
 ...
 */
template<size_t Base, class T>
BigInt<Base,T>::BigInt(const char* n) {
  std::vector<T> digits;
  /// Comprobar si el número es negativo
  int j = 0;
  if (n[0] == '-') {
 ...
 */
template<size_t Base, class T>
BigInt<Base, T>::BigInt(const BigInt<Base ,T>& n) {
  digits_ = n.digits_;
 ...
 */
template<size_t Base, class T>
BigInt<Base, T>& BigInt<Base, T>::operator=(const BigInt<Base, T>& n) {
  digits_ = n.digits_;
  is_negative_ = n.is_negative_;
  return *this;
}
 ...
 */
template<size_t Base, class T>
std::ostream& operator<<(std::ostream& out, const BigInt<Base, T>&a) {
  if (a.is_negative_) {
    out << "-";
  }
  for (int i = a.digits_.size() - 1; i >= 0; i--) {
 ...
 */
template<size_t Base, class T>
std::istream& operator>>(std::istream& in, BigInt<Base, T>& a) {
  std::string s;
  in >> s;
 ...
 */
template<size_t Base, class T>
char BigInt<Base, T>::operator[](int i) const {
 ...
 */
template<size_t Base, class T>
bool operator==(const BigInt<Base, T>& a, const BigInt<Base, T> & b) {
 ...
 */
template<size_t Base, class T>
bool BigInt<Base, T>::operator!=(const BigInt<Base ,T>& b) const {
  return !(*this == b);
}
 ...
 */
template<size_t Base, class T>
BigInt<Base, T>& BigInt<Base, T>::operator++() {
  std::vector<T> &digits = this->digits_;
 ...
 */
template<size_t Base, class T>
BigInt<Base, T> BigInt<Base, T>::operator++(int) {
  BigInt<Base, T> temp(*this);
  ++(*this);
  return temp;
}
 ...
 */
template<size_t Base, class T>
BigInt<Base, T> operator+(const BigInt<Base, T>& a, const BigInt<Base, T>& b) {
  BigInt<Base, T> c;
  if (a == c || b == c) {
    if (a == c) {
      return b;
 ...
 */
template<size_t Base, class T>
BigInt<Base, T> BigInt<Base, T>::operator-(const BigInt<Base ,T> &b) const {
  BigInt<Base, T> c;
  if (*this == c || b == c) {
    if (*this == c) {
      c = -b;
 ...
 */
template<size_t Base, class T>
BigInt<Base, T> BigInt<Base, T>::operator-() const {
  bool condition = !(is_negative_);
  BigInt<Base ,T> n = *this;
 ...
 */
template<size_t Base, class T>
BigInt<Base, T> power(const BigInt<Base, T>& a, const BigInt<Base, T>& b) {
  BigInt<Base, T> c(1), d;
  if (b.is_negative_) {
...
/**
 * Especialización de plantilla par la clase BigInt<2, bool> 
...
class BigInt<2, bool> {
 public: 
  ...
  BigInt(std::string&);
  BigInt(const char*);
  BigInt(const BigInt<2, bool>&); /// Constructor de copia
  /// Getter
  ...
  operator BigInt<8>(); // código de conversión de binario a octal 
  operator BigInt<10>(); // código de conversión de binario a decimal 
  operator BigInt<16>(); // código de conversión de binario a hexadecimal 
  /// Asignación
  BigInt<2, bool>& operator=(const BigInt<2, bool>&);
  /// Inserción y extracción en flujo
  friend std::ostream& operator<< <2>(std::ostream&, const BigInt<2, bool>&); 
  friend std::istream& operator>> <2>(std::istream&, BigInt<2, bool>&);
  /// Accesor
  bool operator[](int) const; /// Acceso al i-ésimo dígito
  /// Comparación
  friend bool operator== <2>(const BigInt<2, bool>&, const BigInt<2, bool> &); 
  bool operator!=(const BigInt<2, bool>&) const;
  friend bool operator> <2>(const BigInt<2, bool>&, const BigInt<2, bool> &); 
  bool operator>=(const BigInt<2, bool> &) const;
  friend bool operator< <2>(const BigInt<2, bool>&, const BigInt<2, bool>&); 
  bool operator<=(const BigInt<2, bool>&) const;
  /// Incremento/decremento
  BigInt<2, bool>& operator++(); /// Pre-incremento
  BigInt<2, bool> operator++(int); /// Post-incremento
  BigInt<2, bool>& operator--(); /// Pre-decremento
  BigInt<2, bool> operator--(int); /// Pre-decremento 
  /// Operadores Aritméticos
  friend BigInt<2, bool> operator+ <2>(const BigInt<2, bool>&, const BigInt<2, bool>&); 
  BigInt<2, bool> operator-(const BigInt<2, bool> &) const;
  BigInt<2, bool> operator-() const;
  BigInt<2, bool> operator*(const BigInt<2, bool>&) const;
  friend BigInt<2, bool> operator/ <2>(const BigInt<2, bool>&, const BigInt<2, bool>&); 
  BigInt<2, bool> operator%(const BigInt<2, bool>&) const;
  /// Potencia a^b
  friend BigInt<2, bool> power <2>(const BigInt<2, bool>&, const BigInt<2, bool>&);
 private:
 ...
 */
BigInt<2, bool> absolute_sum_2(const BigInt<2, bool>& a, const BigInt<2, bool>& b) {
  std::vector<bool> sum;
 ...
 * @param s : Objeto del tipo string
 */
BigInt<2, bool>::BigInt(std::string& s) {
  std::vector<bool> digits;
  bool is_negative = false;
  /// Comprobar si el número es negativo
  int j = 0;
 ...
 * @param n : Objeto del tipo char*
 */
BigInt<2, bool>::BigInt(const char* n) {
  std::vector<bool> digits;
  /// Comprobar si el número es negativo
  int j = 0;
  bool is_negative = false;
 ...
 * @param n : Objeto del tipo BigInt
 */
BigInt<2, bool>::BigInt(const BigInt<2, bool>& n) {
  digits_ = n.digits_;
}

/**
 * @brief Sobrecarga del operador de asignación de la clase BigInt<2, bool>
 * @param n : Objeto del tipo BigInt
 */
BigInt<2, bool>& BigInt<2, bool>::operator=(const BigInt<2, bool>& n) {
  digits_ = n.digits_;
  return *this;
}
 ...
 */
template<>
std::ostream& operator<< <2>(std::ostream& out, const BigInt<2, bool>& a) {
  for (int i = a.digits_.size() - 1; i >= 0; i--) {
    out << a.digits_[i];
  }
  return out;
 ...
 */
template<>
std::istream& operator>> <2>(std::istream& in, BigInt<2, bool>& a) {
  std::string s;
  in >> s;
  std::vector<bool> digits;
  bool is_negative = false;
 ...
 * @brief Permite obtener el valor en el objeto en la posición indicada
 */
bool BigInt<2, bool>::operator[](int i) const {
  assert(i >= 0 && i < (int)digits_.size());
  return digits_[i];
}
 ...
 * @param b : Objeto del tipo BigInt
 */
bool operator==(const BigInt<2, bool>& a, const BigInt<2, bool> & b) {
 ...
 * @param b : Objeto del tipo BigInt
 */
bool BigInt<2, bool>::operator!=(const BigInt<2, bool>& b) const {
  return !(*this == b);
}
 ...
 * @brief Permite aumentar en una unidad el objeto invocante
 */
BigInt<2, bool>& BigInt<2, bool>::operator++() {
 ...
 * @brief Permite aumentar en una unidad el objeto invocante
 */
BigInt<2, bool> BigInt<2, bool>::operator++(int) {
  BigInt<2, bool> temp(*this);
  ++(*this);
  return temp;
}
 ...
 * @param b : Objeto del tipo BigInt
 */
BigInt<2, bool> operator+(const BigInt<2, bool>& a, const BigInt<2, bool>& b) {
  BigInt<2, bool> c, d, e;
 ...
 * @param b : Objeto del tipo BigInt
 */
BigInt<2, bool> BigInt<2, bool>::operator-(const BigInt<2, bool> &b) const {
  BigInt<2, bool> c, d;
  d = -b;
  c = *this + d;
  return c;
 ...
 * @brief Permite cambiar el signo del objeto invocante
 */
BigInt<2, bool> BigInt<2, bool>::operator-() const {
  ...
  BigInt<2, bool> number_to_return;
 ...
 * @param b : Objeto del tipo BigInt
 */
BigInt<2, bool> power(const BigInt<2, bool>& a, const BigInt<2, bool>& b) {
  BigInt<2, bool> c(1), d, e;
  if (b.digits()[b.digits().size() - 1]) {
    throw std::invalid_argument("Representación entera!");
  }
  e = a;
 ...
 */
template<size_t Base, class T>
BigInt<Base ,T>::operator BigInt<2, bool>() {
  BigInt<2, bool> result;
  BigInt<Base ,T> temp = *this, num_2(2), num_0; // Crea un objeto temporal de BigInt<Base ,T> igual al objeto actual
  if (is_negative_) {
    temp = -temp;
  }
 ...
 */
BigInt<2, bool>::operator BigInt<8>() {
  BigInt<8> result;
  BigInt<2, bool> actual_num = *this;
  bool condition = false;
 ...
 */
BigInt<2, bool>::operator BigInt<10>() {
  BigInt<2, bool> actual_num = *this;
  BigInt<10> result;
 ...
 */
BigInt<2, bool>::operator BigInt<16>() {
  BigInt<16> result;
  BigInt<2, bool> actual_num = *this;
  bool condition = false;
  if (digits()[digits().size() - 1]) {
...
/**
 * Multiply Base 2 
 * @brief: Permite multiplicar dos números en base 2, dados sus dígitos en un vector de bool.
 * @param a: número en base 2, representación C2.
 * @param b: número en base 2, representación C2.
 */
BigInt<2, bool> multiply(const BigInt<2, bool>& a, const BigInt<2, bool>& b) {
  BigInt<2, bool> c = a, d = b;
  if (a.digits()[a.digits().size() - 1]) {
    c = -c;
  }
  if (b.digits()[b.digits().size() - 1]) {
  ...
  std::vector<bool> a_digits(c.digits()), b_digits(d.digits());
  int n = a_digits.size(), m = b_digits.size();
  ...
  std::vector<bool> result(res.size());
 ...
 * @param b: número en base 2, representación C2.
 */
BigInt<2, bool> divide(const BigInt<2, bool>& a, const BigInt<2, bool>& b) {
  std::vector<bool> dividend = a.digits(), divisor = b.digits();
  if (a.digits()[a.digits().size() - 1]) {
    dividend = compl_vector(dividend);
  }
  if (b.digits()[b.digits().size() - 1]) {
    divisor = compl_vector(divisor);
  }
  // Obtener el número de bits necesarios
  int dividend_bits = dividend.size();
  int divisor_bits = divisor.size();

  // Añadir ceros a la izquierda del divisor hasta que tenga el mismo número de bits que el dividendo
  std::vector<bool> temp_divisor(divisor_bits + dividend_bits - 1);
  std::copy(divisor.begin(), divisor.end(), temp_divisor.end() - divisor_bits);
    
  // Inicializar el resultado y el resto
  std::vector<bool> result(dividend_bits);

  // Bucle principal
  for (int i = dividend_bits - 1; i >= 0; i--) {
    // Desplazar el divisor un bit a la izquierda y agregar el siguiente bit del dividendo
    std::rotate(temp_divisor.rbegin(), temp_divisor.rbegin() + 1, temp_divisor.rend());
    temp_divisor[dividend_bits] = dividend[i];

    // Restar el divisor del número temporal si es posible
    BigInt<2, bool> c, d;
    c.set_digits(temp_divisor);
    d.set_digits(divisor);
    if (c >= d) {
      temp_divisor = (c - d).digits();
      result[i] = true;
    } else {
      result[i] = false;
    }
  ...
  // // Copiar el resto si lo hay
  // if (temp_divisor.any()) {
  //   remainder.resize(divisor_bits - 1);
  //   std::copy(temp_divisor.begin(), temp_divisor.end() - 1, remainder.begin());
  // }
