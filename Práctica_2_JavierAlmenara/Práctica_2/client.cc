    ...
    case 2: {
      ...
      while (getline(file, line_selected)) {
        ...
        if (item == all_numbers.end() && (isdigit(aux[0])))  {
          ...
          BigInt<2> number = calculator.EvaluateExpression(false);
          all_numbers.insert(std::pair<std::string, BigInt<2>>(tag, number));
        }
      }
      ...
      while (getline(file, line_selected)) {
        ...
        stream_line_aux >> aux;
        stream_line_aux >> aux;
        bool work_in_base_two = false;
        if (aux == "?") {
          work_in_base_two = true;
        }
        stream_line_aux >> aux;
        if (item == all_numbers.end() && (isdigit(aux[0]) || (aux[0] == '-')))  {
          BigInt<8> number{aux};
          all_numbers.insert(std::pair<std::string, BigInt<8>>(tag, number));
        } else {
          Calculator<8> calculator {line_selected, all_numbers};
          BigInt<8> number = calculator.EvaluateExpression(work_in_base_two);
          all_numbers.insert(std::pair<std::string, BigInt<8>>(tag, number));
        }
      }
      file_out << "Base = " << base << std::endl;
      ...
      while (getline(file, line_selected)) {
        ...
        if (item == all_numbers.end() && (isdigit(aux[0]) || (aux[0] == '-'))) {
          ...
          BigInt<10> number = calculator.EvaluateExpression(work_in_base_two);
          all_numbers.insert(std::pair<std::string, BigInt<10>>(tag, number));
        }
      }
      file_out << "Base = " << base << std::endl;
      ...
      while (getline(file, line_selected)) {
        ...
        for (int i = 0; i < (int)aux.size(); i++) {
          if ((!isdigit(aux[i]) && (aux[i] < 'A' || aux[i] > 'F')) && (aux[i] != '-')) {
            std::cout << "<" << std::endl;
            condition = false;
          }
        }
        std::cout << tag << " " << aux << std::endl;
        if (item == all_numbers.end() && condition)  {
          ...
          BigInt<16> number = calculator.EvaluateExpression(work_in_base_two);
          all_numbers.insert(std::pair<std::string, BigInt<16>>(tag, number));
        }
      }
      file_out << "Base = " << base << std::endl;
